...
export function controllerReadiness(controller = {}, catalog = null) {
  const normalized = normalizeProviderConfig(controller);
  ...
  if (provider === "opencode") {
    ...
    if (healthState === "broken_environment") {
      return { state: "broken_environment", message: "OpenCode environment is broken. Repair or re-login first." };
    }
    if (healthState === "not_installed") {
      return { state: "broken_environment", message: "OpenCode CLI is not installed." };
    }
    return { state: "login_required", message: "Login to OpenCode in the terminal/browser, then refresh credentials and models." };
  }
  ...
  if (provider === "openai" || provider === "anthropic" || provider === "gemini-api") {
    ...
    if (!apiKey) {
      return { state: "missing_api_key", message: `Enter an API key for ${providerDisplayName(provider)}.` };
    }
    if (!model) {
  ...
  if (provider === "gemini-cli") {
    ...
    return { state: "ready", message: "Gemini CLI will use existing CLI auth." };
  }
...
export function simulationReadiness(settings, providerCatalogs = {}) {
  ...
  const groupStates = groups.map((group) => {
    const readiness = controllerReadiness(group.controller, providerCatalogs[`group:${group.id}`] ?? null);
    return {
      id: group.id,
      name: group.name,
...
function reconcileGroups(baseScenario, groups = []) {
  ...
  return sourceGroups.map((group, index) => normalizeGroup(baseScenario, group, index, usedIds, validOriginalIds[index]));
}
