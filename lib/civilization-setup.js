...
export const DIRECT_API_PROVIDERS = ["openai", "anthropic", "gemini-api", "groq"];

export const PROVIDER_OPTIONS = [
  { value: "heuristic", label: "Heuristic (debug)" },
  { value: "gemini-cli", label: "Gemini CLI (browser login)" },
  { value: "gemini-api", label: "Gemini API (direct key)" },
  { value: "openai", label: "OpenAI API" },
  { value: "anthropic", label: "Anthropic API" },
  { value: "groq", label: "Groq API" },
  { value: "opencode", label: "OpenCode Zen (managed runtime)" }
];
...
export function applyProviderCapabilities(config) {
  ...
  if (config.provider === "opencode") {
    return {
      ...config,
      executionMode: config.executionMode === "per_human" ? "group_batch" : (config.executionMode || "group_batch"),
      supportsModelListing: true,
      supportsManualModelEntry: true
    };
  }
  ...
  if (config.provider === "groq") {
    return {
      ...config,
      availableModels: config.availableModels.length ? config.availableModels : [...GROQ_MODELS],
      supportsModelListing: true,
      supportsManualModelEntry: true
    };
  }
...
export function controllerReadiness(controller = {}, catalog = null, globalProviders = {}) {
  ...
  if (provider === "openai" || provider === "anthropic" || provider === "gemini-api" || provider === "groq") {
