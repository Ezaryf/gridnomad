...
function buildOpencodeManualCommands({ cliHome = "", credential = "" } = {}) {
  if (!cliHome) {
    return {
      powershell: { login: "", verify: "", models: "" },
      cmd: { login: "", verify: "", models: "" }
    };
  }
...
async function inspectSingleOpencode({ credential = "", cliHome = "", isolated = false } = {}) {
  ...
  const health_state = detectOpencodeHealthState({ authResult, modelsResult: combinedModelsResult, credentials, models });
...
export async function inspectOpencode({ credential = "", cliHome = "", isolated = false } = {}) {
  return inspectSingleOpencode({ credential, cliHome, isolated });
}
...
export async function createManagedOpencodeHome({ currentCliHome = "" } = {}) {
  ...
  const inspection = await inspectOpencode({ cliHome });
  return {
